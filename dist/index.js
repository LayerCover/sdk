...
exports.LayerCoverSDK = exports.NoQuotesAvailableError = exports.RateTooHighError = exports.DEFAULT_API_BASE_URL = exports.DEFAULT_CHAIN_ID = exports.DEPLOYMENT_FALLBACK_CONFIGS = exports.CONTRACT_ADDRESSES = exports.TOKEN_LOGOS = exports.POOL_CONFIG = void 0;
...
exports.DEPLOYMENT_FALLBACK_CONFIGS = {
    base_sepolia_usdc: {
        chainId: 84532,
        contracts: {
            policyManager: '0xbd0Cb34253c84201F746F0A9DF062d82c0823c56',
 ...
 */
exports.DEFAULT_CHAIN_ID = 11155111;
/**
 ...
 */
class RateTooHighError extends errors_1.LayerCoverSDKError {
 ...
 */
class NoQuotesAvailableError extends errors_1.LayerCoverSDKError {
...
class LayerCoverSDK {
    constructor(providerOrSigner, policyManagerAddress, options = {}) {
        ...
        this._apiBaseUrl = (options.apiBaseUrl || exports.DEFAULT_API_BASE_URL).replace(/\/+$/, '');
        ...
        this._poolRegistryAddress = options.poolRegistryAddress || exports.CONTRACT_ADDRESSES[this._chainId]?.poolRegistry;
        this._purchaseGatewayAddress = options.purchaseGatewayAddress || exports.CONTRACT_ADDRESSES[this._chainId]?.purchaseGateway;
        this._quoteBookExtensionAddress = options.quoteBookExtensionAddress || exports.CONTRACT_ADDRESSES[this._chainId]?.quoteBookExtension;
        this._systemRegistryAddress = options.systemRegistryAddress || exports.CONTRACT_ADDRESSES[this._chainId]?.systemRegistry;
    ...
    _buildPurchaseBlockedError(blockers) {
        const message = blockers.length > 0
            ...
            : 'Purchase is blocked.';
        return new errors_1.PurchaseBlockedError(message, blockers);
    }
    ...
    static _getFallbackDeploymentConfig(deployment) {
        if (!deployment)
            return undefined;
        return exports.DEPLOYMENT_FALLBACK_CONFIGS[deployment];
    }
    ...
    static _configFallback(options) {
        const apiBaseUrl = (options.apiBaseUrl || exports.DEFAULT_API_BASE_URL).replace(/\/+$/, '');
        if (options.deployment) {
    ...
    static async fetchConfig(options = {}) {
        const apiBase = (options.apiBaseUrl || exports.DEFAULT_API_BASE_URL).replace(/\/+$/, '');
        // Build query params
        ...
        resolvedChainId = resolvedChainId || exports.DEFAULT_CHAIN_ID;
        resolvedDeployment =
            resolvedDeployment ||
        ...
        const chainFallbackAddresses = exports.CONTRACT_ADDRESSES[resolvedChainId] || {};
        contracts = {
            ...contracts,
    ...
    static async create(providerOrSigner, options = {}) {
        const requestedApiBase = (options.apiBaseUrl || exports.DEFAULT_API_BASE_URL).replace(/\/+$/, '');
    ...
    async prepareBuyFromQuoteTx(orderId, coverageAmount, durationSeconds, referralCode) {
        ...
        const purchaseGatewayIface = new ethers_v6_1.ethers.Interface(LayerCoverSDK.PURCHASE_GATEWAY_ABI);
        return {
    ...
    async purchase(poolId, coverageAmount, durationWeeks, maxRateBps, referralCode) {
        ...
        this._assertInteger('durationWeeks', durationWeeks, 1);
        if (!this.signer)
            throw new errors_1.SignerRequiredError('Signer required for purchase');
    ...
    async purchaseQuote(quote, coverageAmount, durationWeeks, maxRateBps, referralCode) {
        ...
        if (!liveQuote) {
            throw new errors_1.QuoteStaleError(`Quote ${quote.id} is no longer available and must be refreshed before purchase.`, quote.id, DEFAULT_QUOTE_STALE_MS, quoteAgeMs);
        }
        const blockers = this._getQuoteExecutionBlockers(liveQuote, coverageAmount, durationWeeks, maxRateBps);
        if (blockers.length > 0) {
            if (maxRateBps !== undefined && blockers.some((blocker) => blocker.code === 'RATE_TOO_HIGH')) {
    ...
    _encodeQuoteBookPurchaseRequest(quoteId, coverageAmount, durationSeconds, referralCode) {
        return ethers_v6_1.ethers.AbiCoder.defaultAbiCoder().encode([
            'tuple(uint256 quoteId, uint256 coverageAmount, uint64 duration, bytes32 referralCode, address vault, uint256 sharesToCover, bytes extensionData)',
                ...
                referralCode,
                ethers_v6_1.ethers.ZeroAddress,
                0n,
                '0x',
            ]]);
    }
    async _resolveQuoteBookExtensionAddress(quote) {
        ...
        if (candidateFromQuote && candidateFromQuote !== ethers_v6_1.ethers.ZeroAddress) {
            this._quoteBookExtensionAddress = candidateFromQuote;
            return candidateFromQuote;
        }
        if (this._quoteBookExtensionAddress && this._quoteBookExtensionAddress !== ethers_v6_1.ethers.ZeroAddress) {
            return this._quoteBookExtensionAddress;
        }
        if (!this._systemRegistryAddress || this._systemRegistryAddress === ethers_v6_1.ethers.ZeroAddress) {
        ...
        const registry = new ethers_v6_1.Contract(this._systemRegistryAddress, LayerCoverSDK.SYSTEM_REGISTRY_ABI, this.provider);
        ...
        if (!extensionAddress || extensionAddress === ethers_v6_1.ethers.ZeroAddress) {
    ...
    async _resolvePurchaseGatewayAddress(quote) {
        if (this._purchaseGatewayAddress && this._purchaseGatewayAddress !== ethers_v6_1.ethers.ZeroAddress) {
            return this._purchaseGatewayAddress;
        }
        const quoteBookExtensionAddress = await this._resolveQuoteBookExtensionAddress(quote);
        const quoteBookExtension = new ethers_v6_1.Contract(quoteBookExtensionAddress, LayerCoverSDK.QUOTE_BOOK_EXTENSION_RUNTIME_ABI, this.provider);
        ...
        if (!gatewayAddress || gatewayAddress === ethers_v6_1.ethers.ZeroAddress) {
    ...
    async _previewDirectQuoteBookPurchase(quote, coverageAmount, durationSeconds) {
        ...
        const purchaseGateway = new ethers_v6_1.Contract(purchaseGatewayAddress, LayerCoverSDK.PURCHASE_GATEWAY_ABI, this.provider);
    ...
    async _getTokenAllowance(tokenAddress, owner, spender) {
        const tokenContract = new ethers_v6_1.Contract(tokenAddress, ['function allowance(address owner, address spender) view returns (uint256)'], this.provider);
    ...
    async _executeDirectQuoteBookPurchase(quote, coverageAmount, durationSeconds, normalizedReferralCode) {
        ...
        const purchaseGateway = new ethers_v6_1.Contract(purchaseGatewayAddress, LayerCoverSDK.PURCHASE_GATEWAY_ABI, this.signer);
        ...
        if (allowance < preview.premiumDeposit) {
            ...
            const approveTx = await tokenContract.approve(purchaseGatewayAddress, ethers_v6_1.ethers.MaxUint256);
            this._emitEvent('approval_submitted', {
                quoteId: quote.id,
                txHash: approveTx.hash,
                approvalAmount: ethers_v6_1.ethers.MaxUint256.toString(),
    ...
    _extractPolicyResultFromReceipt(receipt) {
        const iface = new ethers_v6_1.ethers.Interface([
    ...
    _createChainMismatchError(connectedChainId) {
        ...
        return new errors_1.ChainMismatchError(`Chain mismatch: SDK configured for ${this._chainId}${deploymentSuffix}, signer connected to ${connectedChainId}`, this._chainId, connectedChainId, this._deployment);
    }
